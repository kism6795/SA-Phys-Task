function ttc = MATB_XML(subj_num, test_date)
%% MATB XML Generator
%{ 
...
subject_folder =  sprintf('Subject-%s-%s',subj_num,test_date);

write_files = 'on';
XML_file_name = fullfile('..\',data_folder,subject_folder,'MATB_EVENTS_MATLAB.xml');
Event_file_name = fullfile('..\',data_folder,subject_folder,'EVENTS_MATLAB.txt');
...
rng(1); % set seed (for now at least)
rate_offset = 15;
...
taskLoad = [zeros(floor(nTrials/2),1); ones(floor(nTrials/2),1)];
motivation = repmat([1;0],(floor(nTrials/2)),1);
memoryLoad = repmat([1;1;0;0],(floor(nTrials/4)),1);
...
%}
nEvents = 9; % baseline # of primary events
...
for i = 1:(totalLength+1)
    ...
    j = 1;
    while events.times(i) > rate_times(j)
       j = j+1; 
    end
